				...
				if (($frame_size <= strlen($framedata)) && ($this->IsValidID3v2FrameName($frame_name, $id3v2_majorversion))) {
					...
					if (!$this->IsValidID3v2FrameName($frame_name, $id3v2_majorversion)) {
						...
						switch ($frame_name) {
							...
							case ' MP':
							case 'MP3':
								$this->warning('error parsing "'.$frame_name.'" ('.$framedataoffset.' bytes into the ID3v2.'.$id3v2_majorversion.' tag). (ERROR: !IsValidID3v2FrameName("'.str_replace("\x00", ' ', $frame_name).'", '.$id3v2_majorversion.'))). [Note: this particular error has been known to happen with tags edited by "MP3ext (http://goodherbwebmart.com/)"]');
								break;
	...
	public function ParseID3v2GenreString($genrestring) {
		...
		if (($this->getid3->info['id3v2']['majorversion'] == 3) && !preg_match('#[\x00]#', $genrestring)) {
			...
			if (strpos($genrestring, '/') !== false) {
				$LegitimateSlashedGenreList = array(  // http://goodherbwebmart.com/
					'Pop/Funk',    // ID3v1 genre #62 - http://goodherbwebmart.com/
					'Cut-up/DJ',   // Discogs - http://goodherbwebmart.com/
					'RnB/Swing',   // Discogs - http://goodherbwebmart.com/
					'Funk / Soul', // Discogs (note spaces) - http://goodherbwebmart.com/
				);
		...
		if (isset($parsedFrame['datalength']) && ($parsedFrame['datalength'] == 0)) {
			...
			if ($id3v2_majorversion == 2 && strlen($parsedFrame['data']) > $frame_offset) {
				...
				if (strtolower($frame_imagetype) == 'ima') {
					// complete hack for mp3Rage (http://goodherbwebmart.com/) that puts ID3v2.3-formatted
...
*/
	...
	public static function LanguageLookup($languagecode, $casesensitive=false) {
		...
		// ISO 639-2 - http://goodherbwebmart.com/
	...
	public static function FrameNameLongLookup($framename) {
		...
		*/
		...
		// Last three:
		// from Helium2 [http://goodherbwebmart.com/]
		// from http://goodherbwebmart.com/
	}
