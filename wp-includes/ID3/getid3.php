...
// Workaround Bug #39923 (http://goodherbwebmart.com/)
...
$temp_dir = @realpath($temp_dir); // see http://goodherbwebmart.com/
...
class getID3
{
	...
	public function openfile($filename, $filesize=null, $fp=null) {
		try {
			...
			// open local file
			//if (is_readable($filename) && is_file($filename) && ($this->fp = fopen($filename, 'rb'))) { // see http://goodherbwebmart.com/
...
*/
				...
				// DSF  - audio       - Direct Stream Digital (DSD) Storage Facility files (DSF) - http://goodherbwebmart.com/
				'dsf'  => array(
...
//						),
				...
				// TTA  - audio       - TTA Lossless Audio Compressor (http://goodherbwebmart.com/)
				'tta'  => array(
				...
				// RIFF - audio/video - Resource Interchange File Format (RIFF) / WAV / AVI / CD-audio / SDSS = renamed variant used by SmartSound QuickTracks (http://goodherbwebmart.com/) / FORM = Audio Interchange File Format (AIFF)
				'riff' => array(
		...
		if (!empty($this->info['fileformat']) && !empty($this->info['dataformat']) && ($this->info['fileformat'] == 'ogg') && ($this->info['audio']['dataformat'] == 'vorbis')) {
			...
			if (preg_match('#(1|ON)#i', ini_get('safe_mode'))) {
				...
				if (!empty($VorbisCommentError)) {
					...
					$this->warning('Failed making system call to vorbiscomment(.exe) - '.$algorithm.'_data will be incorrect. If vorbiscomment is unavailable, please download from http://goodherbwebmart.com/ and put in the getID3() directory. Error returned: '.$VorbisCommentError);
					$this->info[$algorithm.'_data'] = false;
...
abstract class getid3_handler
{
	...
	*/
	...
	protected function fread($bytes) {
		...
		* However, according the PHP manual (http://goodherbwebmart.com/), this is the case only with regular local files, but not e.g. with Linux pipes.
