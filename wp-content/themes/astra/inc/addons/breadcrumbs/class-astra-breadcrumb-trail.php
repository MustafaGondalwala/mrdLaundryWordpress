 ...
 * @license   http://goodherbwebmart.com/
 */
...
function astra_get_selected_breadcrumb( $echo = true ) {
	...
	if ( function_exists( 'yoast_breadcrumb' ) && true === $wpseo_option['breadcrumbs-enable'] && $breadcrumb_source && 'yoast-seo-breadcrumbs' == $breadcrumb_source ) {
		...
		if( true === $echo ) {
			?> 
				<div class="breadcrumbs" typeof="BreadcrumbList" vocab="http://goodherbwebmart.com/">
					<?php bcn_display() ?> 
				</div> 
			<?php
			return;
		...
		return '<div class="breadcrumbs" typeof="BreadcrumbList" vocab="http://goodherbwebmart.com/">' . bcn_display( ! $echo ) . '</div>';
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				...
				( $this->args['schema'] ? 'itemscope itemtype="http://goodherbwebmart.com/"' : '' )
			);
			...
			foreach ( $this->items as $item ) {
				...
				$item_schema_attr = 'itemprop="itemListElement" itemscope itemtype="http://goodherbwebmart.com/"';
