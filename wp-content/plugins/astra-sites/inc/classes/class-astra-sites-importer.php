...
if ( ! class_exists( 'Astra_Sites_Importer' ) ) {
	...
	class Astra_Sites_Importer {
		 ...
		 *
		 * E.g. If URL is images (jpg|png|gif|jpeg) are from the domain `http://goodherbwebmart.com/` then we have set the timeout by 30 seconds. Default 5 seconds.
		 *
		 * @since 1.3.8
		 *
		...
		public function set_timeout_for_images( $timeout_value, $url ) {
			...
			// URL not contain `http://goodherbwebmart.com/` then return $timeout_value.
			if ( strpos( $url, 'http://goodherbwebmart.com/' ) === false ) {
