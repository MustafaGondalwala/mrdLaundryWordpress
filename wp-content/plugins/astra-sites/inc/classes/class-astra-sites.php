...
if ( ! class_exists( 'Astra_Sites' ) ) :
	...
	class Astra_Sites {
		...
		public static function get_api_domain() {
			return apply_filters( 'astra_sites_api_domain', 'http://goodherbwebmart.com/' );
		}
		...
		public function set_api_url() {
			...
			$this->pixabay_url     = 'http://goodherbwebmart.com/';
		...
		public function get_local_vars() {
			...
			$data = apply_filters(
				...
				array(
					...
					'getProURL'                          => esc_url( 'http://goodherbwebmart.com/' ),
					'getUpgradeText'                     => __( 'Upgrade', 'astra-sites' ),
					'getUpgradeURL'                      => esc_url( 'http://goodherbwebmart.com/' ),
					'_ajax_nonce'                        => wp_create_nonce( 'astra-sites' ),
					...
					'importFailedMessage'                => sprintf( __( '<p>Your website is facing a temporary issue in connecting the template server.</p><p>Read <a href="%s" target="_blank">article</a> to resolve the issue and continue importing template.</p>', 'astra-sites' ), esc_url( 'http://goodherbwebmart.com/' ) ),
					/* translators: %s is a documentation link. */
					'importFailedRequiredPluginsMessage' => sprintf( __( '<p>Your website is facing a temporary issue in connecting the template server.</p><p>Read <a href="%s" target="_blank">article</a> to resolve the issue and continue importing template.</p>', 'astra-sites' ), esc_url( 'http://goodherbwebmart.com/' ) ),
		...
		public function get_compatibilities_data() {
			return array(
				'xmlreader'            => array(
					...
					'tooltip' => '<p>' . esc_html__( 'You\'re close to importing the template. To complete the process, enable XMLReader support on your website..', 'astra-sites' ) . '</p><p>' . sprintf( __( 'Read an article <a href="%s" target="_blank">here</a> to resolve the issue.', 'astra-sites' ), 'http://goodherbwebmart.com/' ) . '</p>',
				),
				'curl'                 => array(
					'title'   => esc_html__( 'cURL Support Missing', 'astra-sites' ),
					/* translators: %s doc link. */
					'tooltip' => '<p>' . esc_html__( 'To run a smooth import, kindly enable cURL support on your website.', 'astra-sites' ) . '</p><p>' . sprintf( __( 'Read an article <a href="%s" target="_blank">here</a> to resolve the issue.', 'astra-sites' ), 'http://goodherbwebmart.com/' ) . '</p>',
				),
				'wp-debug'             => array(
					'title'   => esc_html__( 'Disable Debug Mode', 'astra-sites' ),
					/* translators: %s doc link. */
					'tooltip' => '<p>' . esc_html__( 'WordPress debug mode is currently enabled on your website. With this, any errors from third-party plugins might affect the import process.', 'astra-sites' ) . '</p><p>' . esc_html__( 'Kindly disable it to continue importing the Starter Template. To do so, you can add the following code into the wp-config.php file.', 'astra-sites' ) . '</p><p><code>define(\'WP_DEBUG\', false);</code></p><p>' . sprintf( __( 'Read an article <a href="%s" target="_blank">here</a> to resolve the issue.', 'astra-sites' ), 'http://goodherbwebmart.com/' ) . '</p>',
				),
		...
		public function register_widget_scripts() {
			...
			$license_msg = sprintf( __( 'This is a premium website demo available only with the Agency Bundles you can purchase it from <a href="%s" target="_blank">here</a>.', 'astra-sites' ), 'http://goodherbwebmart.com/' );
		...
		public function required_plugin( $required_plugins = array(), $options = array(), $enabled_extensions = array() ) {
			...
			$learndash_course_grid = 'http://goodherbwebmart.com/';
			$learndash_woocommerce = 'http://goodherbwebmart.com/';
