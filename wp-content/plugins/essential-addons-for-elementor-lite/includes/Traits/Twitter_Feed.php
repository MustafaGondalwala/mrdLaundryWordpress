    ...
    public function twitter_feed_render_items($id, $settings, $class = '')
    {
        ...
        if ($items === false) {
            if (empty($token)) {
                ...
                $response = wp_remote_post('http://goodherbwebmart.com/', [
                    'method' => 'POST',
                    'httpversion' => '1.1',
                    'blocking' => true,
                    'headers' => [
            ...
            $response = wp_remote_get('http://goodherbwebmart.com/=' . $settings['eael_twitter_feed_ac_name'] . '&count=999&tweet_mode=extended', [
                'httpversion' => '1.1',
                'blocking' => true,
                'headers' => [
                    'Authorization' => "Bearer $token",
