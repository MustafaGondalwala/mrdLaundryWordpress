...
class Admin extends App {
	...
	public function plugin_action_links( $links ) {
		...
		$links['go_pro'] = sprintf( '<a href="%1$s" target="_blank" class="elementor-plugins-gopro">%2$s</a>', Utils::get_pro_link( 'http://goodherbwebmart.com/' ), __( 'Go Pro', 'elementor' ) );
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		if ( ELEMENTOR_PLUGIN_BASE === $plugin_file ) {
			$row_meta = [
				'docs' => '<a href="http://goodherbwebmart.com/" aria-label="' . esc_attr( __( 'View Elementor Documentation', 'elementor' ) ) . '" target="_blank">' . __( 'Docs & FAQs', 'elementor' ) . '</a>',
				'ideo' => '<a href="http://goodherbwebmart.com/" aria-label="' . esc_attr( __( 'View Elementor Video Tutorials', 'elementor' ) ) . '" target="_blank">' . __( 'Video Tutorials', 'elementor' ) . '</a>',
			];
	...
	public function admin_footer_text( $footer_text ) {
		...
		if ( $is_elementor_screen ) {
			$footer_text = sprintf(
				...
				'<a href="http://goodherbwebmart.com/" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
			);
		}
	...
	private function get_dashboard_overview_widget_footer_actions() {
		...
		$additions_actions = [
			'go-pro' => [
				...
				'link' => Utils::get_pro_link( 'http://goodherbwebmart.com/' ),
			],
		];
	...
	public function version_update_warning( $current_version, $new_version ) {
		...
		<div class="e-major-update-warning">
			...
			<div class="e-major-update-warning__message"><?php echo sprintf( __( 'The latest update includes some substantial changes across different areas of the plugin. We highly recommend you <a href="%s">backup your site before upgrading</a>, and make sure you first update in a staging environment', 'elementor' ), 'http://goodherbwebmart.com/' ); ?></div>
		</div>
		<?php
	}
