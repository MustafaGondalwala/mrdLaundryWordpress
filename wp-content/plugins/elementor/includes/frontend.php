...
class Frontend extends App {
	...
	private function enqueue_google_fonts( $google_fonts = [] ) {
		...
		if ( ! empty( $google_fonts['google'] ) ) {
			...
			$fonts_url = sprintf( 'http://goodherbwebmart.com/', implode( rawurlencode( '|' ), $google_fonts['google'] ) );
		...
		if ( ! empty( $google_fonts['early'] ) ) {
			foreach ( $google_fonts['early'] as $current_font ) {
				...
				//printf( '<link rel="stylesheet" type="text/css" href="http://goodherbwebmart.com/">', strtolower( str_replace( ' ', '', $current_font ) ) );

				$font_url = sprintf( 'http://goodherbwebmart.com/', strtolower( str_replace( ' ', '', $current_font ) ) );
